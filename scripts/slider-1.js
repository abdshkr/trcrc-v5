...
var slider_plugin = (function() {
  ...
  var fifi_slider = function (settings) {
    ...
    _.def = {
      target: $('.n-slider'),
      dotsWrapper: $('.n-dots-wrapper'),
      arrowLeft: $('.n-arrow-left'),
  ...
  fifi_slider.prototype.init = function () {
    ...
    _.def.target.innerHTML = '<div class="n-slider-inner">' + nowHTML + '</div>';

    _.allSlides = 0;
    _.curSlide = 0;
    _.curLeft = 0;
    _.totalSlides = _.def.target.querySelectorAll('.n-slide').length;

    _.sliderInner = _.def.target.querySelector('.n-slider-inner');
    _.loadedCnt = 0;

    // append clones
    var cloneFirst = _.def.target.querySelectorAll('.n-slide')[0].cloneNode(true);
    _.sliderInner.appendChild(cloneFirst);
    var cloneLast = _.def.target.querySelectorAll('.n-slide')[_.totalSlides - 1].cloneNode(true);
    _.sliderInner.insertBefore(cloneLast, _.sliderInner.firstChild);
   
    _.curSlide++;
    _.allSlides = _.def.target.querySelectorAll('.n-slide');
  ...
  fifi_slider.prototype.updateSliderDimension = function () {
    ...
    _.slideW = parseInt(_.def.target.querySelectorAll('.n-slide')[0].offsetWidth);
